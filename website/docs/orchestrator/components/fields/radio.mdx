# Les questions à choix unique

Le composant Radio Permet de sélectionner une valeur parmi une liste d'options (questions à choix unique).  
Les modalités de réponse peuvent être personnalisées.

Il est également possible de proposer un complément textuel (ajout d'un champ de réponse en cas de sélection d'une modalité "Autres").

Les modalités de réponse sont numérotées de 1 à 9 (A à Z s'il y a plus de 9 modalités).  
Les touches du clavier (chiffres/lettres) permettent de sélectionner la modalité associée.

La sélection d'une modalité provoque le passage automatique à la page suivante.

En pied de page, des boutons "Ne sait pas" et "Refus" permettent d'indiquer une non réponse et ainsi poursuivre le déroulement du questionnaire.  
La sélection d'une modalité de non réponse déselectionne la modalité de réponse préalablement sélectionnée, et provoque le passage automatique à la page suivante.  
En revenant sur la page, si une modalité de non réponse était sélectionnée, la sélection d'une modalité de réponse déselectionne la modalité de non réponse.

### Comment faire dans Pogues ?

https://inseefr.github.io/Bowie/pogues/Le%20guide/
