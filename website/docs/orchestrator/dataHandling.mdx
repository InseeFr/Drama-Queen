...
A chaque changement de page du questionnaire, on actualise le contenu d'une interrogation, que ce soit pour persister sa valeur actualisée en collecte ou pour pouvoir la télécharger en visualisation.

L'interrogation est représentée par un objet au format suivant :
...
Dans la mise à jour de l'interrogation, on s'intéresse aux 2 parties :
...
Le stateData est facultatif pour une interrogation. C'est notamment le cas pour une nouvelle interrogation en collecte, ou en visualisation sans avoir renseigné de fichier de data.  
