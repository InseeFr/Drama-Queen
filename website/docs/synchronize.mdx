...
- récupération de la liste des campagnes pour lesquelles l'utilisateur au moins une interrogation qui lui est affectée
...
| ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------ |
| Récupération<ul><li>d'une campagne</li><li>d'un modèle de questionnaire</li><li>d'une interrogation</li><li>d'une nomenclature</li></ul> | format non reconnu | Arrêt de la synchronisation                                                                                        |
| Récupération d'une interrogation                                                                                                         | 400, 403, 404, 500 | Unité non récupérée, poursuite de la synchronisation                                                               |
| Envoi d'une interrogation                                                                                                                | 400, 403, 404, 500 | Unité non mise à jour, suppression de l'unité en locale (déplacée en "temp zone"), poursuite de la synchronisation |
| Envoi d'une interrogation en "temp zone"                                                                                                 | 400, 403, 404, 500 | Arrêt de la synchronisation                                                                                        |
...
| Type | Route                                 | Description              | Sécurisé par Bearer token | Exemple de réponse |
| ---- | ------------------------------------- | ------------------------ | ------------------------- | ------------------ |
| GET  | `/api/survey-unit/${interrogationId}` | Récupère l'interrogation | oui                       | `json {}`          |
| PUT  | `/api/survey-unit/${interrogationId}` | Envoie l'interrogation   | oui                       | `json {}`          |
