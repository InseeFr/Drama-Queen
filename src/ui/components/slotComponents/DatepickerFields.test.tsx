...
import { type DateFormat, DatepickerFields } from './DatepickerFields'

describe('DatepickerFields', () => {
  ...
  dateFormats.forEach((format) => {
    it(`should render properly with format ${format}`, () => {
      const { getByRole } = render(
        <DatepickerFields {...defaultProps} dateFormat={format} />,
      )
  ...
  it('handle change correctly for format YYYY-MM-DD', () => {
    const { getByRole } = render(
      <DatepickerFields {...defaultProps} dateFormat="YYYY-MM-DD" />,
    )
  ...
  it('handle change correctly for format YYYY', () => {
    const { getByRole } = render(
      <DatepickerFields {...defaultProps} dateFormat="YYYY" value="2024" />,
    )
