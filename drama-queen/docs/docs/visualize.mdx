...
La page de visualisation de questionnaire permet de visualiser n'importe quel questionnaire.
...
La validation du formulaire renvoie vers l'orchestrateur pour visualiser le questionnaire, en tenant compte des paramètres fournis.
...
**pas de description du fonctionnement de l'orchestrateur en mode de visu, ajouter ici un lien vers une autre page de la doc quand ça sera fait**
...
Si les données de réponse n'ont pas pu être récupérées, ou qu'elles ne respectent pas le format attendu, on ouvre le questionnaire comme s'il n'y avait pas de données renseignées.
Il en est de même pour chacune des nomenclatures renseignées.
