...
Nb : Pour limiter la taille des données, on retire toutes les variables collectées qui ont une valeur _null_ ; elles ne sont pas nécessaires pour la passation du questionnaire, même en cas de réouverture du questionnaire.  
...
Si le stateData n'est pas défini, à l'ouverture du questionnaire on l'initialise à
...
- `currentPage` : prend la valeur du `lastReachedPage` de Lunatic, c'est à dire la dernière page atteinte
